...
- **Anti-cheat / proctoring simulation** — detects tab-switching or window-blur via the `visibilitychange` API and surfaces a warning modal plus a persistent on-screen banner
...
| A02 – Security Misconfiguration | CORS (3), Host Header Attacks (5), Info Disclosure (5) | ✅ 13/13 |
...
| A10 – Mishandling of Exceptional Conditions | Verbose error handling / exception leakage | ✅ Done |

## Adding a new page

1. Drop the finished HTML file into the right `pages/aXX-.../` folder.
2. Open `data.js`, find that category's `subs` array, add or update an object:
   ```js
